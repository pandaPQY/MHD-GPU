...
int
setupCL_transpose13u1234_ProgramKernel()
{
    ...
    file[3]=fopen("transpose13u1234_Kernels.cl","rb");
    int i;
...
//
    transpose13u1234_Program = clCreateProgramWithSource(
		mhdComputeContext,
	        number_of_file,
	        &source[0],
	        sourceSize,
    ...
    {
	printf("Error: Failed to create program for transpose13u1234_Kernels.cl!\n");
        return MHD_FAILURE;
    }
...
//int cl_build_err=clBuildProgram(clprog, 0, NULL, buffer, NULL, NULL);
	...
	char buffer[2048];
	size_t len;
	clGetProgramBuildInfo(transpose13u1234_Program, mhdComputeDevice[0], CL_PROGRAM_BUILD_LOG, sizeof(buffer), buffer, &len);
	printf("%s\n", buffer);
	printf("Error: Failed to build program executable for transpose13u1234_Kernels.cl!\n");
	return MHD_FAILURE;
    }

    /* get a kernel object handle for a kernel with the given name */
    transpose13u1234_Kernel = clCreateKernel(
		transpose13u1234_Program,
                "transpose13u1234",
                &status_in_setupCL);
    if (status_in_setupCL != CL_SUCCESS)
    {
	printf("Error: Failed to create kernel for transpose13u1234_Kernels.cl!\n");
	return MHD_FAILURE;
    }
...
int
setupCL_transpose13u5b_ProgramKernel()
{
...
//
    ...
    file[1]=fopen("parameter.h","rb");
    file[2]=fopen("array_definition.h","rb");
    file[3]=fopen("transpose13u5b_Kernels.cl","rb");
    int i;
    for (i=0; i<number_of_file; i++)
    {
        if (file[i] == NULL)
...
//
    transpose13u5b_Program = clCreateProgramWithSource(
                mhdComputeContext,
                number_of_file,
                &source[0],
                sourceSize,
    ...
    if (status_in_setupCL != CL_SUCCESS)
    {
        printf("Error: Failed to create program for transpose13u5b_Kernels.cl!\n");
        return MHD_FAILURE;
    }
    status_in_setupCL = clBuildProgram(
                transpose13u5b_Program,
                1,
                &mhdComputeDevice[0],
                NULL,
                NULL,
    ...
    if (status_in_setupCL != CL_SUCCESS)
    {
        char buffer[2048];
        size_t len;
        clGetProgramBuildInfo(transpose13u5b_Program, mhdComputeDevice[0], CL_PROGRAM_BUILD_LOG, sizeof(buffer), buffer, &len);
        printf("%s\n", buffer);
        printf("Error: Failed to build program executable for transpose13u5b_Kernels.cl!\n");
        return MHD_FAILURE;
    }

    /* get a kernel object handle for a kernel with the given name */
    transpose13u5b_Kernel = clCreateKernel(
                transpose13u5b_Program,
                "transpose13u5b",
                &status_in_setupCL);
    if (status_in_setupCL != CL_SUCCESS)
    {
        printf("Error: Failed to create kernel for transpose13u5b_Kernels.cl!\n");
        return MHD_FAILURE;
    }
//
        return MHD_SUCCESS;
...
int
setupCL_transpose13u1234_ArgumentKernel()
{
    cl_int status_in_setupCL = CL_SUCCESS;
...
int
setupCL_transpose13u5b_ArgumentKernel()
{
    cl_int status_in_setupCL = CL_SUCCESS;
