...
int
MHD::setupCL_transpose13_ProgramKernel()
{
    ...
    kernelFile[3].open("transpose13_Kernels.cl");
    int i;
...
//
    transpose13_Program = clCreateProgramWithSource(
		mhdComputeContext,
	        number_of_file,
	        &source[0],
	        sourceSize,
    ...
    {
	printf("Error: Failed to create program for transpose13_Kernels.cl!\n");
        return MHD_FAILURE;
    }
...
//int cl_build_err=clBuildProgram(clprog, 0, NULL, buffer, NULL, NULL);
	...
	char buffer[2048];
	size_t len;
	clGetProgramBuildInfo(transpose13_Program, mhdComputeDevice[0], CL_PROGRAM_BUILD_LOG, sizeof(buffer), buffer, &len);
	printf("%s\n", buffer);
	printf("Error: Failed to build program executable for transpose13_Kernels.cl!\n");
	return MHD_FAILURE;
    }

    /* get a kernel object handle for a kernel with the given name */
    transpose13_Kernel = clCreateKernel(
		transpose13_Program,
                "transpose13",
                &status_in_setupCL);
    if (status_in_setupCL != CL_SUCCESS)
    {
	printf("Error: Failed to create kernel for transpose13_Kernels.cl!\n");
	return MHD_FAILURE;
    }
...
int
MHD::setupCL_transpose13_ArgumentKernel()
{
    cl_int status_in_setupCL = CL_SUCCESS;
