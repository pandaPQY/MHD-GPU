...
! x again
   ...
   call fluidx(u,b,nx,ny,nz,dt)
!   call output
   if (mod(iter,10) .eq. 1) write(*,*) 't=',t,iter,maxval(u(1,:,:,:)),minval(u(1,:,:,:))
end do
print*,'above is the result of n=',n,' iter=',iter
print*,''
!close(101)
!call output
...
! background : rho=B_x=1 all others zero
  ...
  b(2,:,:,:)=-u(3,:,:,:)                                                   !drag                      
  b(3,:,:,:)=-u(4,:,:,:)                                                   !drag
  b(2,:,:,:)=(b(2,:,:,:)+cshift(b(2,:,:,:),-1,2))/2                        !drag
  b(3,:,:,:)=(b(3,:,:,:)+cshift(b(3,:,:,:),-1,3))/2                        !drag
...
!  return
  ...
  enddo
  b(2,:,:,:)=b(2,:,:,:)+u(2,:,:,:)/sqrt(2.)                                !drag
  u(1,:,:,:)=u(1,:,:,:)+u(2,:,:,:)
  u(5,:,:,:)=u(5,:,:,:)+sum(b**2,1)/2+sum(u(2:4,:,:,:)**2,1)/u(1,:,:,:)/2
  u(5,:,:,:)=u(5,:,:,:)+p0*u(2,:,:,:)*5./3./(2./3.)
  write(*,*) 'magnetosonic'
!  open(99,file='source64.dat',access='stream')
!  write(99) u
!  close(99)
  !!!!!!!!!!!!!!!drag
!  b=0
...
!  return
  ...
  do k=1,nz
     do i=1,nx
        u(3,i,:,k)= -0.1*cos( 2*3.14159*i/nx + 2*3.14159*k/nz )
        u(2,i,:,k)= -0.1*sin( 2*3.14159*i/nx + 2*3.14159*k/nz )/sqrt(2.0)
        u(4,i,:,k)=  0.1*sin( 2*3.14159*i/nx + 2*3.14159*k/nz )/sqrt(2.0)
     enddo
  enddo
  do k=1,nz
     do i=1,nx
        b(2,i,:,k)=  0.1*cos( 2*3.14159*i/nx + 2*3.14159*k/nz )
        b(1,i,:,k)=  (1.0 + 0.1*sin( 2*3.14159*i/nx + 2*3.14159*k/nz))/sqrt(2.0)
        b(3,i,:,k)=  (1.0 - 0.1*sin( 2*3.14159*i/nx + 2*3.14159*k/nz))/sqrt(2.0)
     enddo
  enddo
!  u(5,:,:,:)=1.0 + 0.5 + 0.1*0.1;  ! to keep things stable
u(5,:,:,:)=1.5+0.01!u(5,:,:,:)+sum(b**2,1)/2+sum(u(2:4,:,:,:)**2,1)/u(1,:,:,:)/2
end subroutine init
