module works
!implicit none
!!$omp declare target(n,nx,ny,nz,u,b,t,dt,iter)
...
contains
  subroutine output
    ...
    do j=1,1
    do i=1,nx
       write(10,'(i5,7(1x,e10.3))') i,u(1,i,j,1)-1,u(2:4,i,j,1)/u(1,i,j,1),b(:,i,j,1)-(/1,0,0/)
    end do
    do i=1,nx
...
! locals
    ...
    gamma=5./3.
    c=0
    !$omp parallel shared(u,b) num_threads(nthreads)
    !$omp do collapse(3) private(kp,jp,ip,bx,by,bz,v,ps,p,b2) reduction(max:c) !num_threads(nthreads)
!   !$omp simd private(kp,jp,ip,bx,by,bz,v,ps,p,b2) reduction(max:c)
    do k=1,nz
       do j=1,ny
          do i=1,nx
             kp=mod(k,nz)+1
...
!  ub(6:8,:,:,:)=b
!  write(*,*) 'alven wave'
!  fn='source_init_alfvenlinear_'//trim(adjustl(npd))//'cube.dat'
!  open(99,file=trim(fn),status='replace',access='stream')
!  write(99) ub
!  close(99)
!  return
  ! magnetic
...
!  ub(1:5,:,:,:)=u
!  ub(6:8,:,:,:)=b
!  open(99,file='source.dat',status='replace',access='stream')
!  write(99) ub
!  close(99)
end subroutine init
...
!  implicit none
  ...
  real tmpb(nx)
  real, dimension(3,nx,ny,nz) :: b3xx
  integer i,ip
  !$omp parallel shared(b,u,dt,nx,ny,nz) num_threads(nthreads)
  !$omp do collapse(2) private(b3x,jp,kp)! shared(b,u,dt,nx,ny,nz) !num_threads(nthreads)
!   !$omp simd private(u1x,b3x,jp,kp)! shared(b,u,dt,nx,ny,nz)
  do k=1,nz
!    !$omp simd private(b3x,jp,kp)
     do j=1,nx
        b3x=b(:,:,j,k)/2
        jp=mod(j,ny)+1
        kp=mod(k,nz)+1
...
!  implicit none
  ...
  integer j,k,jm,km
  !$omp parallel shared(nx,nz,ny,b,dt) num_threads(nthreads)
  !$omp do collapse(2) private(k,j,jm,vx,b1x,fluxbx)! shared(nx,nz,ny,b,dt) num_threads(nthreads)
!  !$omp do simd  private(jm,vx,b1x,fluxbx)
  do k=1,nz
!    !$omp do simd! safelen(n)
     do j=1,ny
        jm=mod(j+ny-2,ny)+1
...
!$omp end parallel
!$omp parallel shared(nx,nz,ny,b,dt) num_threads(nthreads)
   !$omp do collapse(2) private(k,j,km,vx,b1x,fluxbx)! shared(nx,ny,nz,b,dt) num_threads(nthreads) 
! !$omp  simd private(km,vx,b1x,fluxbx)
  do k=1,nz
 !    !$omp do simd! safelen(n)
...
!$omp declare target
!!$omp declare simd(tvdb) uniform(vg,n,dt)
...
!$omp declare target
!!$omp declare simd(tvd1) uniform(n,dt)
!  implicit none
  integer n
  real dt
  real u(5,n),b(3,n)
...
!$omp declare target
!!$omp declare simd(mhdflux) uniform(u,b,n)
!  implicit none
  integer n
...
!$omp parallel  shared(u,b,ny,nx,nz,ut,bt) num_threads(nthreads)
!$omp do private(i,j,u2,b2)! num_threads(nthreads)
!!$omp do private(k) !private(i,j,u2,b2) num_threads(nthreads)
!!$omp simd private(u2,b2)
  do k=1,nz
 !   !$omp do simd linear(j:1) 
!!$omp parallel  shared(u,b,ny,nx,nz,ut,bt) num_threads(nthreads)
!!$omp do collapse(2) private(i,j,u2,b2)! num_threads(nthreads)
     do j=1,ny
!       !$omp do simd linear(i:1)
        do i=1,nx
         u2(:,j,i)=u((/1,3,2,4,5/),i,j,k)
         b2(:,j,i)=b((/2,1,3/),i,j,k)
!           ut(:,j,i,k)=u((/1,3,2,4,5/),i,j,k)
!           bt(:,i,j,k)=b((/2,1,3/),i,j,k)
        end do
     end do
!!$omp end parallel
     ut(:,:,:,k)=u2
...
!  implicit none
  ...
  if (nx .eq. nz) then
!$omp parallel shared(u,b,ny,nx,nz,ut,bt) num_threads(nthreads)
!$omp do  private(i,k,u2,b2)! num_threads(nthreads)
!!$omp do private(j)! num_threads(nthreads)
!!$omp simd private(u2,b2)
  do j=1,ny
!!$omp parallel shared(u,b,ny,nx,nz,ut,bt) num_threads(nthreads)
!     !$omp do simd linear(k:1)
!!$omp do collapse(2) private(i,k,u2,b2)! num_threads(nthreads)
     do k=1,nz
!         !$omp do simd linear(i:1)
        do i=1,nx
           u2(:,k,i)=u((/1,4,3,2,5/),i,j,k)
           b2(:,k,i)=b((/3,2,1/),i,j,k)
 !          ut(:,k,j,i)=u((/1,4,3,2,5/),i,j,k)
 !          bt(:,k,j,i)=b((/3,2,1/),i,j,k)
        end do
     end do
!!$omp end parallel
     ut(:,:,j,:)=u2
