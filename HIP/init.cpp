...
//	read the source_init.dat for the data
  ...
  //init_File=fopen("source_init_alfvenlinear_32cube.dat","r+");
  //init_File=fopen("source_init_alfvenlinear_64cube.dat","r+");
  ...
  for (i=0;i<(*init_nx);i++)
  {
	for (j=0;j<(*init_ny);j++)
	{
		for (k=0;k<(*init_nz);k++)
		{
			look=fscanf(init_File,"%f",&init_u[a4D_FinC(5,(*init_nx),(*init_ny),(*init_nz),(1-1),i,j,k)]);
			look=fscanf(init_File,"%f",&init_u[a4D_FinC(5,(*init_nx),(*init_ny),(*init_nz),(2-1),i,j,k)]);
			look=fscanf(init_File,"%f",&init_u[a4D_FinC(5,(*init_nx),(*init_ny),(*init_nz),(3-1),i,j,k)]);
			look=fscanf(init_File,"%f",&init_u[a4D_FinC(5,(*init_nx),(*init_ny),(*init_nz),(4-1),i,j,k)]);
			look=fscanf(init_File,"%f",&init_u[a4D_FinC(5,(*init_nx),(*init_ny),(*init_nz),(5-1),i,j,k)]);
			look=fscanf(init_File,"%f",&init_b[a4D_FinC(3,(*init_nx),(*init_ny),(*init_nz),(1-1),i,j,k)]);
			look=fscanf(init_File,"%f",&init_b[a4D_FinC(3,(*init_nx),(*init_ny),(*init_nz),(2-1),i,j,k)]);
			look=fscanf(init_File,"%f",&init_b[a4D_FinC(3,(*init_nx),(*init_ny),(*init_nz),(3-1),i,j,k)]);
		}
	}
  }
  fclose(init_File);
  printf("init: %f, %f \n",init_u[nu*total_cell_number],init_b[8191]);
  printf("init: %f, %f \n",init_u[nu*total_cell_number-1],init_b[8190]);
//	finished
printf("end in init\n");
return;
/*
